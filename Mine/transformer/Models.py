...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.self_attent_bias = nn.ModuleList(
            [FrequencyLayer(0.1, d_model, 10) for _ in range(n_layers)]
        )
    ...
    def forward(self, event_type, event_time, non_pad_mask):
        ...
        for freq_layer in self.self_attent_bias:
            freq_output = freq_layer(freq_output)
            freq_output = freq_output + tem_enc
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        # self.alpha = nn.Parameter(torch.full((num_types, ), -0.1))

        # parameter for the softplus function
        self.beta = nn.Parameter(torch.tensor(1.0))
        # self.beta = nn.Parameter(torch.full((num_types, ), 1.0))
    ...
    def forward(self, event_type, event_time):
        ...
        enc_output = self.encoder(event_type, event_time, non_pad_mask)  # Event-wise attention
