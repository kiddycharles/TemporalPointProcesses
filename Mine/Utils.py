...
from Mine.transformer.Models import get_non_pad_mask
...
def compute_integral_unbiased(model, data, time, non_pad_mask, type_mask):
    ...
    # print("timeshape: ", time.shape)  # [batch_size, time_len]
    num_samples = 100

    diff_time = (time[:, 1:] - time[:, :-1]) * non_pad_mask[:, 1:]  # keep batch axis, time differences.

    # print("diff_timeshape: ", diff_time.shape) # [batch_size, time_len - 1], since difference are intervals
    #
    # print("diff_timeshape_unsqueeze(2): ", diff_time.unsqueeze(2).shape)  # unsqueeze(2) add another axis
    # print(diff_time)
    # print(diff_time.unsqueeze(2))
    print(torch.rand([*diff_time.size(), num_samples]).size())  # 【batch_size, seq_len, num_samples]
    temp_time = diff_time.unsqueeze(2) * torch.rand([*diff_time.size(), num_samples], device=data.device)
    print(temp_time.size())  # [batch_size, seq_len, num_samples]
...
def log_likelihood(model, data, time, types):
    ...
    all_hid = model.linear(data)  # w^T h
